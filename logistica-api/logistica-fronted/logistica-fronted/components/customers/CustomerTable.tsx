...
export function CustomerTable() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["customers", { page, search, ordering, customer_type }],
    queryFn: () => getCustomers({ page, search, ordering, customer_type }),
  ...
  if (isError) {
    return <p className="text-sm text-destructive">Error al cargar clientes.</p>;
  }
            ...
            <SheetContent side="bottom" className="pb-8">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {TYPE_FILTERS.map((f) => (
                    <Button key={f.value} variant={customer_type === f.value ? "default" : "outline"} size="sm" onClick={() => updateParams({ customer_type: f.value, page: "1" })}>
                      {f.label}
                    </Button>
                  ))}
                </div>

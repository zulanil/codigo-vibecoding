...
export function ShipmentTable() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["shipments", { page, search, ordering, status }],
    queryFn: () => getShipments({ page, search, ordering, status }),
  ...
  const customersMap = new Map<number, string>(
    customersData?.results.map((c) => [c.id, c.name]) ?? []
  );
  ...
  if (isError) {
    return <p className="text-sm text-destructive">Error al cargar envíos.</p>;
  }
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <Input
            ...
            placeholder="Buscar por tracking o destino..."
            value={searchInput}
